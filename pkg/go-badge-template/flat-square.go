package badge_template

var FlatSquareTemplate = `
<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="{{.Bounds.Dx}}" height="20">
	<g shape-rendering="crispEdges">
		<rect width="{{.Bounds.SubjectDx}}" height="20" fill="#555"/>
		<rect x="{{.Bounds.SubjectDx}}" width="{{.Bounds.StatusDx}}" height="20" fill="{{or .Color "#97ca00" | html}}"/>
	</g>
	<g fill="#fff" text-anchor="middle" font-family="Verdana,Geneva,DejaVu Sans,sans-serif" text-rendering="geometricPrecision" font-size="11">
		<image x="5" y="3" width="14" height="14" xlink:href="data:image/svg+xml;base64,"/>
		<text x="{{.Bounds.SubjectX}}" y="14" fill="#fff">{{.Subject | html}}</text>
		<text x="{{.Bounds.StatusX}}" y="14" fill="#fff">{{.Status | html}}</text>
	</g>
</svg>
`
